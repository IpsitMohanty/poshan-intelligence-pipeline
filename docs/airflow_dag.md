...
Trigger `poshan_intelligence_pipeline` from the UI (or `airflow dags trigger poshan_intelligence_pipeline` from inside the scheduler container) with the default `month=2025-11` param. Output lands in `data/2025-11/`, `warehouse/`, and `models/` on the host -- same bind-mount convention `docker-compose.yml` uses for the `etl`/`cube`/`api` services.
